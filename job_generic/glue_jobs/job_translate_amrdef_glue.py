...
def translator(xml_file_path,s3_path_result,spark):
    ...
    s3_path_schema = "s3://primestone-glue-scripts-dev/amrdef-schema/schemaAMRDEF.json"
    #s3_path_result = "s3://primestone-trust-dev/tests/"
    str_sch = spark.read.text(s3_path_schema).first()[0]
    ...
    logger.warn("Writing CSV to {}".format(s3_path_result))
    df_union.write.format('csv').mode("overwrite").save(s3_path_result, header=True)
    ###Testing workflow###
...
if __name__=='__main__':
    ...
    logger = logging.getLogger('translate-job-log')
    ...
    logger.info(args)
    output_file = filename.split('.')[0] + "|translate"
    ...
    logger.warn("ENRICH BUCKET:{}".format(output_bucket))
    logger.warn("ENRICH FILE:{}".format(output_file))
    logger.warn("****************************************************")
    logger.warn("****************************************************")
